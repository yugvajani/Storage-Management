...
def add_cashier_window():
    ...
    name_label = Label(add_window, text="Name of cashier")
    name_label.grid(row=0, column=0, padx=10, pady=10)
    name_entry = Entry(add_window)
    name_entry.grid(row=0, column=1, padx=10, pady=10)
    ...
    number_label = Label(add_window, text="Phone number")
    number_label.grid(row=2, column=0, padx=10, pady=10)
    number_entry = Entry(add_window)
    ...
    add_button = Button(add_window, text="Add",
                        command=lambda: add_cashier_query( number_entry.get(),address_entry.get(),name_entry.get(),
                                                          salary_entry.get(), tip_entry.get(), add_window))
    add_button.grid(row=5, columnspan=2, sticky='nsew')

def add_cashier_query(number,address,name, salary, tip, curr_window):
    curr_window.attributes("-topmost", True)
    try:
        salary = float(salary)
    except:
        messagebox.showerror("Error!", "Enter a number for salary please")
        print( number,address,name, salary, tip)
        return
    ...
    cursor.execute("select eid from cashier;")
    eid = cursor.fetchall()
    e_id = generate_id(eid)
    try:
        query = "INSERT INTO cashier VALUES('{}','{}',{},'{}',{},{});".format(e_id, number,address,name,salary,tip)
        cursor.execute(query)
        db.commit()
...
def update_cashier_window():
    ...
    name_label = Label(update_window, text="Select name of cashier")
    name_label.grid(row=0, column=0, padx=10, pady=10)

    cursor.execute("SELECT name from cashier;")
    ...
    number_label = Label(update_window, text="Phone Number")
    number_label.grid(row=2, column=0, padx=10, pady=10)
    number_entry = Entry(update_window, name='number_entry')
    number_entry.grid(row=2, column=1, padx=10, pady=10)
    ...
    default_product.trace('w', lambda *args, b=address_entry, c=number_entry, d=salary_entry,
                                      e=tip_entry: set_values_product(default_cashier, b, c, d, e, *args))

    update_button = Button(update_window, text="Update",
                           command=lambda: update_cashier_query(default_cashier.get(), address_entry.get(), number_entry.get(),
                                                                salary_entry.get(), tip_entry.get(), update_window))
    update_button.grid(row=5, columnspan=2, sticky='nsew')

def set_values_cashier(selected, address_entry, number_entry, salary_entry, tip_entry, *args):
    print("SELECT * FROM cashier where name = '{}'".format(selected.get()))
    cursor.execute("SELECT * FROM cashier where name = '{}'".format(selected.get()))
    record = cursor.fetchall()
    record = record[0]
    print(record)
    print(record[3])
    ...
    print(record[5])
    print(record[2])
    print(address_entry, number_entry, salary_entry, tip_entry)

    address_entry.delete(0, 'end')
    address_entry.insert(0, record[3])
    number_entry.delete(0, 'end')
    number_entry.insert(0, record[4])
    salary_entry.delete(0, 'end')
    salary_entry.insert(0, record[5])
    tip_entry.delete(0, 'end')
    tip_entry.insert(0, record[2])

def update_cashier_query(name, address, number, salary, tip, curr_window):
    print(name, address, number, salary, tip, curr_window)
    curr_window.attributes("-topmost", True)
    try:
        salary = float(salary)
    except:
        messagebox.showerror("Error!", "Enter a number for salary please")
        print(name, address, number, salary, tip)
        return
    ...
    try:
        query = "UPDATE cashier SET address='{}', number={}, salary={}, tip={} Where " \
                "name='{}';".format(address, number, name, salary, tip)
        cursor.execute(query)
        db.commit()
        messagebox.showinfo("Success", "Product updated!")
        print_result(get_cashier(), root.nametowidget("table_frame"))
